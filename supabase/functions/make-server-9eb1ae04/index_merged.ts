...
const kv = {
  set: async (key: string, value: any): Promise<void> => {
    const { error } = await kvClient().from("kv_store_9eb1ae04").upsert({ key, value });
    if (error) throw new Error(error.message);
  },
  get: async (key: string): Promise<any> => {
    const { data, error } = await kvClient().from("kv_store_9eb1ae04").select("value").eq("key", key).maybeSingle();
    if (error) throw new Error(error.message);
    return data?.value;
  },
  delete: async (key: string): Promise<void> => {
    const { error } = await kvClient().from("kv_store_9eb1ae04").delete().eq("key", key);
    if (error) throw new Error(error.message);
  },
  getByPrefix: async (prefix: string): Promise<any[]> => {
    const { data, error } = await kvClient().from("kv_store_9eb1ae04").select("key, value").like("key", prefix + "%");
    if (error) throw new Error(error.message);
...
app.get("/make-server-9eb1ae04/actions", async (c) => {
  try {
    ...
    if (!firstCard || seedVersion !== "v4") {
      console.log("Seeding action cards one by one (v4)...");
