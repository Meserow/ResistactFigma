...
async function seedEllenUser() {
  ...
  // Seed password comes from an env secret — never hardcode a credential in
  // source. A literal here ends up in git history AND the deployed bundle, and
  // because this email is on ADMIN_EMAILS it's effectively an admin backdoor.
  // If the secret isn't configured we skip auth-user creation entirely (and do
  // NOT set the flag, so it can run later once the secret is set) rather than
  // ship a known password.
  const seedPassword = Deno.env.get("SEED_ELLEN_PASSWORD");
  if (!seedPassword) {
    console.log("Skipping Ellen seed: SEED_ELLEN_PASSWORD not set.");
    return;
  }
  try {
...
app.get("/make-server-9eb1ae04/actions", async (c) => {
  try {
    ...
    // One-time: clear imageContain ("Fit logo / don't crop") on the 17 cards
    // that still had it set. These now carry cartoon banners that fill the
    // frame, so the contain/letterbox treatment is no longer wanted. The
    // matching SEED_CARDS literals have had `imageContain: true` removed too,
    // so re-seeds stay consistent. Handles both action: and user-action: keys.
    const imageContainFitLogoCleared = await getMigrationFlag("cleanup:clear-imagecontain-fitlogo:v1");
    if (!imageContainFitLogoCleared) {
      const fitLogoIds = [1123, 1186, 1266, 1267, 1273, 1274, 1279, 1280, 1281, 1349, 1351, 1352, 1353, 1371, 1372, 1385, 1389];
      let clearedFitLogo = 0;
      for (const id of fitLogoIds) {
        for (const prefix of ["action:", "user-action:"]) {
          const existing = (await kv.get(`${prefix}${id}`)) as any;
          if (existing && typeof existing === "object" && existing.imageContain === true) {
            const cleared = { ...existing };
      ...
      await setMigrationFlag("cleanup:clear-imagecontain-fitlogo:v1");
      console.log(`Cleared imageContain (fit-logo) from ${clearedFitLogo} of ${fitLogoIds.length} cards.`);
    }
...
app.post("/make-server-9eb1ae04/share-invite", async (c) => {
  ...
  try {
    // This endpoint is intentionally usable without an account (the Spread the
    // Word modal advertises "no account required"), so it can't gate on a user
    // JWT. Instead it's hardened against being an open spam/phishing relay:
    // recipients are capped + format-validated, the free-text note is length-
    // capped and always wrapped in our own template (so the mail is recognizably
    // ResistAct and can't carry arbitrary phishing content from our domain), and
    // sends are rate-limited per client IP.
    ...
    if (recipients.length > MAX_RECIPIENTS) {
      return c.json({ error: `Too many recipients (max ${MAX_RECIPIENTS})` }, 400);
    }

    // Per-IP daily cap so one client can't fan out invites at scale.
    const fwd = c.req.header("x-forwarded-for")?.split(",")[0]?.trim();
    const ip = fwd || c.req.header("x-real-ip") || c.req.header("cf-connecting-ip") || "unknown";
    if (!(await allowDaily(ip, "share-invite", 5))) {
      return c.json({ error: "Daily invite limit reached. Try again tomorrow." }, 429);
    }

    const siteUrl = "https://resistact.us";
    // Cap + sanitize the optional personal note, then always wrap it in our own
    // template and canonical link. The caller can personalize but never owns the
    // whole body — this is what stops the endpoint being an arbitrary-content
    // relay sending from noreply@resistact.org.
    const cleanNote = typeof note === "string"
      ? note.replace(/\s+/g, " ").trim().slice(0, 500)
      : "";
    const intro = cleanNote
      ? `${cleanNote}\n\n`
      : "I've been using ResistAct to find small, doable actions every day to push back on what's happening in America. ";
    const body = `${intro}Check it out — new actions every day, pick what fits your schedule and mood!\n\n${siteUrl}`;
