...
const SEED_CARDS = [
  ...
  { id: 17, category: "IRREVERENCE", categoryColor: "#9333ea", actionType: "In Person Group", timeCommitment: "Half day", title: "Puppets for March on Washington", description: "We are making effigies of Trump and his minions for the March on Washington on July 4th. Join in even if you can't attend — we will help the attendees get them!", location: "Austin, TX", boosts: 500, spotsTotal: "Unlimited", authorName: "Adam Jordan", authorRole: "Catholic Legal Immigration Network", authorLink: "https://www.cliniclegal.org/", topImageKey: "imgImage9", authorAvatarKey: "imgImage3" },
...
app.get("/make-server-9eb1ae04/actions", async (c) => {
  try {
    ...
    const orgsSeeded = await kv.get("seed:org-actions:v9");
    if (!orgsSeeded) {
      let count = 0;
...
app.post("/make-server-9eb1ae04/actions/create", async (c) => {
  try {
    ...
    const { title, description, category, categoryColor, location, isOnline, spotsTotal, sponsor, link, vettingInfo, actionType, topImageUrl, imageContain } =
      await c.req.json<{
